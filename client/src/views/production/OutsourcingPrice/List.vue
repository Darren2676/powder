...
import { getOutsourcingPriceLists, getOutsourcingPriceListDetail, createOutsourcingPriceList, updateOutsourcingPriceList, deleteOutsourcingPriceList, exportOutsourcingPriceLists, importOutsourcingPriceList } from '@/api/production/outsourcingPrice'
...
// ==================== 列定义 ====================
const { loading, dataSource, searchText, pagination, fetchData, handleTableChange, handleSearch, handleReset } = useTableList(getOutsourcingPriceLists)
...
const fetchList = async () => {
  loading.value = true
  try {
    const res: any = await getOutsourcingPriceLists({
...
const openCreate = () => {
  modalTitle.value = '新建委外价目表'
  isView.value = false
  formData.value = { price_type: '含税', currency: 'CNY' }
  detailRows.value = []
  modalVisible.value = true
}

const openView = async (record: any) => {
  modalTitle.value = '查看委外价目表'
  isView.value = true
  const res: any = await getOutsourcingPriceListDetail(record.price_list_number)
...
const handleDelete = (record: any) => {
  Modal.confirm({
    title: '确认删除', content: `确定删除委外价目表 ${record.price_list_number}？`,
    icon: () => null,
    onOk: async () => {
      await deleteOutsourcingPriceList(record.price_list_number)
...
const onSupplierSelect = (val: string) => {
  ...
  if (sup) {
    ...
    formData.value.supplier_category = sup.supplier_category || ''
  }
}
...
const handleSave = async () => {
  ...
  if (formData.value.price_list_number) {
    await updateOutsourcingPriceList(formData.value.price_list_number, payload)
    message.success('更新成功')
  } else {
    await createOutsourcingPriceList(payload)
...
// ==================== 审批 ====================
const handleSubmitApproval = async (record: any) => { await submitForApproval('outsourcing_price_list', record.price_list_number); message.success('提交审批成功'); fetchList() }
const handleApprove = async (record: any) => { await approveRecord('outsourcing_price_list', record.price_list_number); message.success('审批通过'); fetchList() }
const handleWithdraw = async (record: any) => { await withdrawApproval('outsourcing_price_list', record.price_list_number); message.success('撤回成功'); fetchList() }
const handleReverse = async (record: any) => { await reverseApproval('outsourcing_price_list', record.price_list_number); message.success('反审批成功'); fetchList() }

// ==================== 导出 ====================
const handleExport = async () => {
  const res: any = await exportOutsourcingPriceLists(searchText.value)
  const url = window.URL.createObjectURL(new Blob([res.data]))
  const link = document.createElement('a')
  link.href = url
  link.setAttribute('download', 'outsourcing_price_lists.xlsx')
...
const handleImportFile = async (e: Event) => {
  ...
  fd.append('file', file)
  try {
    const res: any = await importOutsourcingPriceList(fd)
  ...
  <div style="padding: 20px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        ...
        <a-button @click="handleReset"><template #icon><ReloadOutlined /></template></a-button>
    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" width="1200px" @ok="handleSave" :ok-button-props="{ style: isView ? { display: 'none' } : {} }" :cancel-text="isView ? '关闭' : '取消'">
      <a-form layout="vertical">
        <a-row :gutter="16">
          ...
          <a-col :span="6"><a-form-item label="供应商">
            <a-select v-model:value="formData.supplier_number" show-search option-filter-prop="label" style="width:100%" @change="onSupplierSelect" :disabled="isView" placeholder="选择供应商" allow-clear>
          ...
          <a-col :span="6"><a-form-item label="供应商分类">
            <a-input v-model:value="formData.supplier_category" :disabled="isView" />
          </a-form-item></a-col>
      ...
      <a-table :columns="detailColumns" :data-source="detailRows" :pagination="false" :row-key="(_r: any, i: number) => i" size="small" :scroll="{ x: 1400 }">
