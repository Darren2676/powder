...
import { getHQCockpitOverview } from '@/api/system/headquarters'
...
// 各工厂KPI对比 - 横向条形图
const factoryComparisonOption = computed(() => {
  const fc = ranking.value.factory_comparison || []
  if (fc.length === 0) return {}
  const names = fc.map((r: any) => r.factory_short || r.factory_name)
  return {
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    legend: { data: ['销售额', '采购额', '完成率', '合格率', 'OEE'], top: 0 },
    grid: { left: 80, right: 40, bottom: 30, top: 50 },
    xAxis: { type: 'value' },
    ...
    series: [
      {
        ...
        type: 'bar',
        stack: 'amount',
        data: fc.map((r: any) => r.sales_amount),
        itemStyle: { color: '#1890ff' },
        label: { show: true, position: 'inside', formatter: (p: any) => p.value >= 10000 ? (p.value / 10000).toFixed(1) + '万' : p.value.toString() }
      },
      {
        name: '采购额',
        type: 'bar',
        stack: 'amount',
        data: fc.map((r: any) => r.purchase_amount),
      ...
      },
      {
        ...
        type: 'bar',
        stack: 'rate',
        data: fc.map((r: any) => r.production_rate),
        itemStyle: { color: '#52c41a' },
        label: { show: true, position: 'inside', formatter: (p: any) => p.value ? p.value.toFixed(1) + '%' : '-' }
      },
      {
        name: '合格率',
        type: 'bar',
        stack: 'rate',
        data: fc.map((r: any) => r.quality_rate),
...
// 各工厂销售额对比 - 分组柱状图
const factorySalesBarOption = computed(() => {
  const bd = kpi.value.factory_breakdown || []
  if (bd.length === 0) return {}
  const names = bd.map((r: any) => r.factory_short || r.factory_name)
  return {
    tooltip: { trigger: 'axis' },
    legend: { data: ['销售额', '采购额'], top: 0 },
    grid: { left: 60, right: 30, bottom: 30, top: 40 },
    xAxis: { type: 'category', data: names },
    yAxis: { type: 'value', axisLabel: { formatter: (v: number) => v >= 10000 ? (v / 10000).toFixed(0) + '万' : v.toString() } },
    series: [
      { name: '销售额', type: 'bar', data: bd.map((r: any) => r.sales_amount), itemStyle: { color: '#1890ff' } },
      { name: '采购额', type: 'bar', data: bd.map((r: any) => r.purchase_amount), itemStyle: { color: '#fa8c16' } }
    ]
  }
})
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await getHQCockpitOverview(params)
    data.value = res.data?.data || res.data || {}
  } catch (error) {
    console.error('Failed to fetch HQ cockpit data:', error)
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="cockpit-page">
    ...
    <div class="cockpit-header">
      <h2 class="cockpit-title">集团管理驾驶舱</h2>
      <a-space>
    ...
    <a-spin :spinning="loading">
      ...
      <a-row :gutter="16">
        <a-col :xs="24" :md="12">
          <a-card title="各工厂销售额 vs 采购额" size="small" style="margin-bottom: 16px;">
            <VChart :option="factorySalesBarOption" style="height: 280px;" autoresize />
          </a-card>
        </a-col>
        <a-col :xs="24" :md="12">
          <a-card title="各工厂KPI对比" size="small" style="margin-bottom: 16px;">
            <VChart :option="factoryComparisonOption" style="height: 280px;" autoresize />
          </a-card>
        </a-col>
      </a-row>
      ...
      <a-row :gutter="16">
        <a-col :xs="24" :md="12">
          <a-card title="销售额 vs 采购额 月度趋势（全集团）" size="small" style="margin-bottom: 16px;">
            <VChart :option="salesPurchaseTrendOption" style="height: 280px;" autoresize />
          </a-card>
        </a-col>
        <a-col :xs="24" :md="12">
          <a-card title="生产产出趋势（全集团）" size="small" style="margin-bottom: 16px;">
      ...
      <a-row :gutter="16">
        <a-col :xs="24" :md="8">
          <a-card title="销售订单状态分布（全集团）" size="small" style="margin-bottom: 16px;">
            <VChart :option="salesStatusOption" style="height: 260px;" autoresize />
          </a-card>
        </a-col>
        <a-col :xs="24" :md="8">
          <a-card title="生产单阶段分布（全集团）" size="small" style="margin-bottom: 16px;">
            <VChart :option="prodFunnelOption" style="height: 260px;" autoresize />
          </a-card>
        </a-col>
        <a-col :xs="24" :md="8">
          <a-card title="不合格品处理分布（全集团）" size="small" style="margin-bottom: 16px;">
      ...
      <a-row :gutter="16">
        <a-col :xs="24" :md="12">
          <a-card title="客户销售额 TOP10（全集团）" size="small" style="margin-bottom: 16px;">
            <VChart :option="customerTop10Option" style="height: 300px;" autoresize />
          </a-card>
        </a-col>
        <a-col :xs="24" :md="12">
          <a-card title="供应商采购额 TOP10（全集团）" size="small" style="margin-bottom: 16px;">
      ...
      <!-- 第六行: 待审批汇总 -->
      <a-card title="待审批单据汇总（全集团）" size="small" style="margin-bottom: 16px;">
        <a-table
...
export default { name: 'HQCockpitPage' }
</script>
