...
import { getPendingRequestDetails, exportPendingRequestDetailsSelected } from '@/api/sales/shippingRequest'
...
const defaultDataColumns: any[] = [
  { title: '状态', dataIndex: 'request_status', key: 'request_status', width: 90, resizable: true },
  ...
  { title: '本次发货', dataIndex: 'ship_quantity', key: 'ship_quantity', width: 100, resizable: true },
  ...
  { title: '申请人', dataIndex: 'creation_man', key: 'creation_man', width: 100, resizable: true },
  { title: '申请日期', dataIndex: 'request_date', key: 'request_date', width: 170, resizable: true }
]
...
} = useColumnPreference('shipping_pending_details', defaultDataColumns, {
  fixedLeft: [{ title: '申请单号', dataIndex: 'request_number', key: 'request_number', width: 170, fixed: 'left' as const, resizable: true }],
  fixedRight: []
})

const statusColors: Record<string, string> = {
  '待审核': 'orange',
  [APPROVAL_STATUS.APPROVED]: 'blue',
  '已发货': 'green',
  '已取消': 'default'
}
...
const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await getPendingRequestDetails({
...
const handleExportSelected = async () => {
  ...
  try {
    const res = await exportPendingRequestDetailsSelected({ ids: selectedRowKeys.value })
    const blob = new Blob([res.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = generateExportFilename('shipping_request_details_selected')
    link.click()
...
<template>
  <div style="padding: 20px">
    <div style="margin-bottom: 16px; display: flex; gap: 8px; align-items: center; flex-wrap: wrap">
      <span style="font-size: 18px; font-weight: 600; color: #1a1a2e; margin-right: 4px; white-space: nowrap">发货申请明细</span>
      <a-input-search
        v-model:value="searchText"
        placeholder="搜索申请单号/产品/客户/销售订单号"
        style="width: 320px"
        @search="handleSearch"
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'request_status'">
          <a-tag :color="statusColors[record.request_status] || 'default'">{{ record.request_status }}</a-tag>
        </template>
        <template v-else-if="column.key === 'ship_quantity'">
          <span style="color: #1677ff; font-weight: 600">{{ record.ship_quantity }}</span>
        </template>
