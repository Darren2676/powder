...
const fetchData = async () => {
  ...
  try {
    ...
    const supplierRes: any = results[2].status === 'fulfilled' ? results[2].value : null
    ...
    if (supplierRes?.success) {
      const rows = supplierRes.data || []
      supplierChartOption.value = {
        tooltip: {
          trigger: 'axis',
        ...
        xAxis: {
          type: 'category',
          data: rows.map((r: any) => r.supplier_name),
    ...
    if (deliveryRes?.success) {
      ...
      deliveryChartOption.value = {
        ...
        legend: { top: 0, data: ['涉及订单数', '交货数量'], textStyle: { fontSize: 11 } },
        grid: { left: '3%', right: '5%', top: 28, bottom: '4%', containLabel: true },
        xAxis: {
          type: 'category',
  ...
  <div class="dashboard-container">
    ...
    <a-tabs v-model:activeKey="activeTab" type="card">
      <a-tab-pane key="overview" tab="仪表板概览">
        <a-spin :spinning="loading">
          ...
          <a-row :gutter="12" class="section-row">
            ...
            <a-col :xs="24" :md="14">
              <a-card :headStyle="{ fontWeight: 600, padding: '0 16px', minHeight: '36px', fontSize: '13px' }" :bodyStyle="{ padding: '6px' }" title="供应商订单金额 TOP10">
                <VChart :option="supplierChartOption" style="height: 220px;" autoresize />
              </a-card>
            </a-col>
          </a-row>
          ...
          <a-row :gutter="12" class="section-row">
            ...
            <a-col :xs="24" :md="10">
              <a-card :headStyle="{ fontWeight: 600, padding: '0 16px', minHeight: '36px', fontSize: '13px' }" :bodyStyle="{ padding: '6px' }" title="未来交货趋势 (未来8周)">
      ...
      <a-tab-pane key="detail" tab="订单明细">
        <a-spin :spinning="loading">
          <a-card :headStyle="{ fontWeight: 600, padding: '0 16px', minHeight: '36px', fontSize: '13px' }" :bodyStyle="{ padding: '0' }" title="最近采购订单">
            <a-table
            ...
            >
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'order_status'">
                  <a-tag :color="orderStatusColorMap[record.order_status] || 'default'" style="margin: 0;">{{ record.order_status }}</a-tag>
                </template>
