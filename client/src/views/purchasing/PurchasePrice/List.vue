...
const openView = async (record: any) => {
  ...
  isView.value = true
  const res: any = await getPurchasePriceListDetail(record.price_list_number)
...
const addDetailRow = () => {
  ...
  detailRows.value.push({
    ...
    pricing_unit: '', max_price_inclusive: 0, max_price_exclusive: 0,
    min_price_inclusive: 0, min_price_exclusive: 0, remark: ''
  })
}
...
const handleSave = async () => {
  ...
  if (formData.value.price_list_number) {
    await updatePurchasePriceList(formData.value.price_list_number, payload)
    message.success('更新成功')
  } else {
    await createPurchasePriceList(payload)
...
const handleExport = async () => {
  ...
  link.href = url
  link.setAttribute('download', 'purchase_price_lists.xlsx')
...
const handleImportFile = async (e: Event) => {
  ...
  fd.append('file', file)
  try {
    const res: any = await importPurchasePriceList(fd)
