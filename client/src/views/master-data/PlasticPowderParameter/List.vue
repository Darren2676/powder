...
import {
  ...
  DownloadOutlined, UploadOutlined, PlusOutlined, CheckOutlined, UndoOutlined, EyeOutlined,
...
import {
  ...
  approvePlasticPowderParameter, withdrawPlasticPowderParameter
} from '@/api/master-data/plasticPowderParameter'
...
defineOptions({ name: 'PlasticPowderParameterList' })
...
const { loading, dataSource, searchText, selectedRowKeys, pagination, rowSelection, fetchData, handleTableChange, handleSearch, handleReset } = useTableList(getPlasticPowderParameters)
...
const editForm = reactive<ParamForm>({ param_name: '', param_code: '', unit: '', remark: '' })

// 新增弹窗
const createModalVisible = ref(false)
const { modalStyle: createModalStyle, onDragStart: createDragStart, resetDrag: createResetDrag } = useModalDrag()
const createForm = reactive<ParamForm>({ param_name: '', param_code: '', unit: '', remark: '' })
...
const defaultDataColumns: any[] = [
  { title: '参数名称', dataIndex: 'param_name', key: 'param_name', width: 150, resizable: true },
...
} = useColumnPreference('plastic_powder_parameter_list', defaultDataColumns, {
...
const handleEdit = (record: any) => {
  ...
  Object.assign(editForm, { param_name: record.param_name, param_code: record.param_code, unit: record.unit, remark: record.remark })
  editResetDrag()
  editModalVisible.value = true
}

const handleEditSave = async () => {
  if (!editForm.param_name) { message.warning('参数名称不能为空'); return }
  if (!editForm.param_code) { message.warning('参数编码不能为空'); return }
  modalLoading.value = true
  try {
    const res: any = await updatePlasticPowderParameter(editingId.value!, { ...editForm })
...
// 新增
const handleCreate = () => {
  Object.assign(createForm, { param_name: '', param_code: '', unit: '', remark: '' })
  createResetDrag()
  createModalVisible.value = true
}

const handleCreateSave = async () => {
  if (!createForm.param_name) { message.warning('参数名称不能为空'); return }
  if (!createForm.param_code) { message.warning('参数编码不能为空'); return }
  modalLoading.value = true
  try {
    const res: any = await createPlasticPowderParameter({ ...createForm })
...
const handleDelete = (record: any) => {
  ...
  Modal.confirm({
    ...
    content: `确定要删除参数「${record.param_name}」吗？`,
    okText: '确定',
    cancelText: '取消',
    onOk: async () => {
      try {
        const res: any = await deletePlasticPowderParameter(record.id)
...
const handleApprove = async (record: any) => {
  try {
    const res: any = await approvePlasticPowderParameter(record.id)
...
const handleWithdraw = async (record: any) => {
  Modal.confirm({
    title: '确认撤消',
    icon: createVNode(ExclamationCircleOutlined),
    content: `确定要撤消塑粉参数「${(record.param_name || '').trim()}」的审核吗？`,
    okText: '确定',
    cancelText: '取消',
    async onOk() {
      try {
        const res: any = await withdrawPlasticPowderParameter(record.id)
...
const handleExport = async () => {
  try {
    const res = await exportPlasticPowderParameters(searchText.value)
    const blob = new Blob([res as any], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = generateExportFilename('塑粉参数')
    a.click()
...
const onFileChange = async (e: Event) => {
  ...
  try {
    const res: any = await importPlasticPowderParameters(formData)
...
onMounted(() => { loadColumnPreference(); fetchData(); loadUnitOptions() })
</script>
  ...
  <div style="padding: 0;">
    <a-card :bordered="false" :body-style="{ padding: '16px' }">
      ...
      <div style="margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; flex-wrap: nowrap; overflow-x: auto">
        <span style="font-size: 18px; font-weight: 600; white-space: nowrap; flex-shrink: 0">塑粉参数</span>
        <div style="display: flex; gap: 8px; align-items: center; flex-wrap: nowrap">
          <a-input-search v-model:value="searchText" placeholder="搜索参数名称/编码/单位" @search="handleSearch" style="width: 220px;" allow-clear @change="(e: any) => !e.target.value && handleReset()" />
    ...
    >
      <template #title>
        <div @mousedown="createDragStart" style="cursor: move; user-select: none;">新增塑粉参数</div>
      </template>
      <div>
        <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
          <a-form-item label="参数名称" required>
            <a-input v-model:value="createForm.param_name" placeholder="请输入参数名称" />
          </a-form-item>
          <a-form-item label="参数编码" required>
            <a-input v-model:value="createForm.param_code" placeholder="请输入参数编码" />
          </a-form-item>
          <a-form-item label="单位">
            <a-select v-model:value="createForm.unit" placeholder="选择单位" allow-clear>
              <a-select-option v-for="u in unitOptions" :key="u.unit_code" :value="u.unit_name">
    ...
    >
      <template #title>
        <div @mousedown="editDragStart" style="cursor: move; user-select: none;">编辑塑粉参数</div>
      </template>
      <div>
        <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
          <a-form-item label="参数名称" required>
            <a-input v-model:value="editForm.param_name" placeholder="请输入参数名称" />
          </a-form-item>
          <a-form-item label="参数编码" required>
            <a-input v-model:value="editForm.param_code" placeholder="请输入参数编码" />
          </a-form-item>
          <a-form-item label="单位">
            <a-select v-model:value="editForm.unit" placeholder="选择单位" allow-clear>
              <a-select-option v-for="u in unitOptions" :key="u.unit_code" :value="u.unit_name">
                {{ u.unit_code }} - {{ u.unit_name }}
              </a-select-option>
            </a-select>
    ...
    >
      <template #title>
        <div @mousedown="detailDragStart" style="cursor: move; user-select: none;">塑粉参数详情</div>
      </template>
      <div>
        <a-descriptions bordered :column="1" size="small">
          <a-descriptions-item label="参数名称">{{ detailData.param_name }}</a-descriptions-item>
          <a-descriptions-item label="参数编码">{{ detailData.param_code }}</a-descriptions-item>
