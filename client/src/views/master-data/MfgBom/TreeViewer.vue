...
import { getMfgBomHeaders, getMfgBomTree } from '@/api/master-data/mfgBom'
...
defineOptions({ name: 'MfgBomTreeViewer' })
...
const fetchBomOptions = async () => {
  try {
    const res = await getMfgBomHeaders({ page: 1, limit: 9999 })
  ...
  tree.details.forEach((d: any, index: number) => {
    ...
    if (d.children) {
      if (d.children.circular) {
        ...
        node.circular_bom_number = d.children.bom_number || d.children.mfg_bom_number
      } else {
        node.sub_bom_info = {
          bom_number: d.children.mfg_bom_number || d.children.bom_number,
          bom_name: d.children.mfg_bom_name || d.children.bom_name,
...
function buildEChartsNode(detail: any): any {
  ...
  if (detail.children) {
    if (detail.children.circular) {
      node.children = [{
        name: '循环引用',
        value: detail.children.bom_number || detail.children.mfg_bom_number,
...
const handleLoadTree = async () => {
  if (!selectedBomNumber.value) { message.warning('请选择一个制造BOM'); return }
  ...
  allRowKeys.value = []
  try {
    const res = await getMfgBomTree(selectedBomNumber.value)
    bomTreeRaw.value = (res as any).data
    const nodes = transformBomTreeToTableData((res as any).data, 'root', 0)
    tableTreeData.value = nodes
    const keys = collectAllExpandableKeys(nodes)
    allRowKeys.value = keys
    await nextTick()
    expandedRowKeys.value = [...keys]
  } catch { message.error('加载制造BOM树形结构失败') }
  finally { treeLoading.value = false }
}

const handleExpandAll = () => { expandedRowKeys.value = [...allRowKeys.value] }
const handleCollapseAll = () => { expandedRowKeys.value = [] }
const handleGoBack = () => { router.push('/mfg-boms') }
  ...
  <div class="mfg-bom-tree-page">
    ...
    <a-card v-if="bomTreeRaw" :bordered="false" size="small">
      <a-descriptions :column="4" size="small" :labelStyle="{ fontWeight: 'bold' }">
        <a-descriptions-item label="制造BOM编号">{{ bomTreeRaw.mfg_bom_number }}</a-descriptions-item>
        <a-descriptions-item label="制造BOM名称">{{ bomTreeRaw.mfg_bom_name }}</a-descriptions-item>
        ...
        <a-descriptions-item label="状态">
          <a-tag :color="bomTreeRaw.condition === CONDITION_STATUS.ENABLED ? 'green' : 'red'">{{ bomTreeRaw.condition }}</a-tag>
        </a-descriptions-item>
    ...
    <a-card v-if="!bomTreeRaw && !treeLoading" :bordered="false" class="tree-content-card">
      <a-empty description="请选择一个制造BOM查看其树形结构">
        <template #image>
