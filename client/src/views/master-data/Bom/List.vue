...
const columnFilters = reactive<Record<string, string>>({ bom_number: '', bom_name: '', item_number: '', item_name: '' })
...
const selectedHeaderRecord = computed(() => {
  ...
  return headerData.value.find(h => h.bom_number === selectedHeaderKey.value) || null
})
...
// Watch view mode change to load tree data
...
const defaultHeaderDataColumns: any[] = [
  { title: 'BOM编号', dataIndex: 'bom_number', key: 'bom_number', width: 110, resizable: true, customFilterDropdown: true },
  { title: 'BOM名称', dataIndex: 'bom_name', key: 'bom_name', width: 110, resizable: true, customFilterDropdown: true },
  { title: '产品编号', dataIndex: 'item_number', key: 'item_number', width: 100, resizable: true, customFilterDropdown: true },
  { title: '产品名称', dataIndex: 'item_name', key: 'item_name', width: 100, resizable: true, customFilterDropdown: true },
...
} = useColumnPreference('bom_header_list', defaultHeaderDataColumns, {
...
} = useColumnPreference('bom_detail_list', defaultDetailDataColumns, {
...
const handleHeaderDetail = async (record: BomHeader) => {
  ...
  try {
    // 同时加载明细和树形数据（两个独立的API）
    const [detailRes, treeRes] = await Promise.all([
      getBomHeaderDetail(record.bom_number!),
      getBomTree(record.bom_number!)
    ])
    console.log('[BOM详情] detailRes:', JSON.stringify(detailRes).substring(0, 200))
    console.log('[BOM详情] treeRes:', JSON.stringify(treeRes).substring(0, 200))

    // Tab 2: 物料明细 — 使用 getBomHeaderDetail（与下方明细表使用同一API）
    ...
    console.log('[BOM详情] 物料明细数量:', headerDetailDetails.value.length)

    // Tab 3 & 4: 树形结构 + 可视化图表 — 使用 getBomTree
    ...
    if (rawTree && rawTree.details) {
      ...
      console.log('[BOM详情] 树形节点数:', nodes.length, '可展开:', keys.length)
    }
  } catch (e: any) {
    console.error('[BOM详情] 加载失败:', e)
...
function buildEChartsNode(detail: any): any {
  ...
  if (detail.children) {
    if (detail.children.circular) {
      node.children = [{ name: '循环引用', value: detail.children.bom_number, itemStyle: { color: '#ff4d4f' }, label: { color: '#ff4d4f' } }]
...
const handleMoreAction = async (key: string, record: BomHeader) => {
  const id = record.bom_number!
  if (key === 'edit') {
    ...
    bomRouter.push({ path: '/bom-tree', query: { bomNumber: id } })
  } else if (key === 'history') {
    ...
    Modal.confirm({
      ...
      onOk: async () => { try { await submitForApproval('bom_header', id); message.success('提交审核成功'); fetchHeaders() } catch { message.error('提交审核失败') } }
    })
  } else if (key === 'approve') {
    Modal.confirm({
      title: '审核通过', icon: createVNode(ExclamationCircleOutlined),
      content: '确定审核通过吗？', okText: '通过', cancelText: '取消',
      onOk: async () => { try { await approveRecord('bom_header', id); message.success('审核通过'); fetchHeaders() } catch { message.error('审核失败') } }
    })
  } else if (key === 'withdraw') {
    Modal.confirm({
      title: '撤回提交', icon: createVNode(ExclamationCircleOutlined),
      content: '确定要撤回审核提交吗？', okText: '撤回', cancelText: '取消',
      onOk: async () => { try { await withdrawApproval('bom_header', id); message.success('撤回成功'); fetchHeaders() } catch { message.error('撤回失败') } }
    })
  } else if (key === 'reverse') {
    Modal.confirm({
      ...
      onOk: async () => { try { await reverseApproval('bom_header', id); message.success('反审成功，已退回草稿'); fetchHeaders() } catch { message.error('反审失败') } }
    })
  }
}
...
const handleExport = async (format: string = 'xlsx') => {
  try {
    ...
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = generateExportFilename('bom_data', ext); link.click(); URL.revokeObjectURL(link.href)
      ...
      <a-tabs v-model:activeKey="pageTab" size="small">
        ...
        <a-tab-pane key="header" tab="BOM物料清单（主表）">
          <div class="tab-toolbar">
            <div></div>
            <a-space :size="4" wrap>
              <a-input-search v-model:value="searchText" placeholder="搜索BOM编号/名称/产品" style="width: 170px" size="small" @search="handleHeaderSearch" />
    ...
    <a-modal v-model:open="detailCreateVisible" title="新增物料" @ok="handleDetailCreateOk" okText="确认" cancelText="取消" width="800px">
      <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 14 }">
        <a-row :gutter="16">
          ...
          <a-col :span="12"><a-form-item label="工序号"><a-select v-model:value="detailCreateForm.step_number" show-search allow-clear placeholder="请选择工序"
                :options="procedureOptions" :filter-option="(input: string, option: any) => option.label.toLowerCase().includes(input.toLowerCase())" /></a-form-item></a-col>
        </a-row>
        <a-row :gutter="16">
          <a-col :span="12">
    ...
    <a-modal v-model:open="detailEditVisible" title="编辑物料" @ok="handleDetailEditOk" okText="确认" cancelText="取消" width="800px">
      <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 14 }">
        <a-row :gutter="16">
          ...
          <a-col :span="12"><a-form-item label="工序号"><a-select v-model:value="detailEditForm.step_number" show-search allow-clear placeholder="请选择工序"
                :options="procedureOptions" :filter-option="(input: string, option: any) => option.label.toLowerCase().includes(input.toLowerCase())" /></a-form-item></a-col>
        </a-row>
        <a-row :gutter="16">
          <a-col :span="12">
        ...
        <a-tabs v-model:activeKey="headerDetailTab" :animated="false">
          ...
          <a-tab-pane key="info" tab="基本信息">
            <a-descriptions :column="2" bordered size="small" :labelStyle="{ fontWeight: 'bold', width: '120px' }">
              <a-descriptions-item label="BOM编号">{{ headerDetailRecord.bom_number }}</a-descriptions-item>
              <a-descriptions-item label="BOM名称">{{ headerDetailRecord.bom_name }}</a-descriptions-item>
          ...
          <a-tab-pane key="details" tab="物料明细">
            ...
            >
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'matched_bom_number'">
                  <span v-if="record.matched_bom_number || record.child_bom_number" style="color: #1890ff;"><ApartmentOutlined /> {{ record.child_bom_number || record.matched_bom_number }}</span>
                  <span v-else style="color: #ccc;">-</span>
                </template>
              </template>
            </a-table>
          ...
          <a-tab-pane key="tree" tab="BOM结构树">
            ...
            <a-table
              :columns="treeTableColumns"
              :data-source="headerDetailTreeData"
              childrenColumnName="children"
              v-model:expandedRowKeys="headerDetailTreeExpKeys"
              rowKey="key"
              :pagination="false"
              :scroll="{ x: 1200, y: 480 }"
              size="small"
