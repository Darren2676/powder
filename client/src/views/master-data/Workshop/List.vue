...
import { getWorkshops, createWorkshop, updateWorkshop, deleteWorkshop, exportWorkshops, importWorkshops, toggleWorkshopStatus, approveWorkshop, withdrawWorkshop } from '@/api/master-data/workshop'
...
const { loading, dataSource, searchText, selectedRowKeys, pagination, rowSelection, fetchData, handleTableChange, handleSearch, handleReset } = useTableList<Workshop>(getWorkshops)
...
const handleExport = async () => {
  try {
    ...
    link.download = generateExportFilename('workshops')
    link.click()
...
const handleToggleStatus = async (record: Workshop) => {
  try {
    await toggleWorkshopStatus(record.workshop_number!)
...
const handleApprove = async (record: any) => {
  try {
    const res: any = await approveWorkshop(record.workshop_number!)
...
const handleWithdraw = async (record: any) => {
  Modal.confirm({
    title: '确认撤消',
    icon: createVNode(ExclamationCircleOutlined),
    content: `确定要撤消车间「${(record.workshop_name || '').trim()}」的审核吗？`,
    okText: '确定',
    cancelText: '取消',
    async onOk() {
      try {
        const res: any = await withdrawWorkshop(record.workshop_number!)
