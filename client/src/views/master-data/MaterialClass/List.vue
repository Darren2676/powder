...
import { getMaterialClasses, createMaterialClass, updateMaterialClass, deleteMaterialClass, exportMaterialClasses, importMaterialClasses, approveMaterialClass, withdrawMaterialClass } from '@/api/master-data/materialClass'
...
const { loading, dataSource, searchText, selectedRowKeys, pagination, rowSelection, fetchData, handleTableChange, handleSearch, handleReset } = useTableList<MaterialClass>(getMaterialClasses)
...
const handleApprove = async (record: any) => {
  try {
    const res: any = await approveMaterialClass(record.material_class_number)
...
const handleWithdraw = async (record: any) => {
  Modal.confirm({
    ...
    content: `确定要撤消物料分类「${(record.material_class_name || '').trim()}」的审核吗？`,
    okText: '确定',
    cancelText: '取消',
    async onOk() {
      try {
        const res: any = await withdrawMaterialClass(record.material_class_number)
...
const handleExport = async () => {
  try {
    ...
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = generateExportFilename('material_class'); link.click(); URL.revokeObjectURL(link.href)
