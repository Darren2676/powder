...
import { getProductClasses, createProductClass, updateProductClass, deleteProductClass, exportProductClasses, importProductClasses } from '@/api/productClass'
...
const dataSource = ref<ProductClass[]>([])
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getProductClasses({ page: pagination.current, limit: pagination.pageSize, search: searchText.value || undefined })
    if (res.success) {
...
const handleExport = async () => {
  try {
    ...
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'product_class.xlsx'; link.click(); URL.revokeObjectURL(link.href)
