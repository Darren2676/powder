...
import { getProcedures, createProcedure, updateProcedure, deleteProcedure, exportProcedures, importProcedures } from '@/api/procedure'
...
const dataSource = ref<Procedure[]>([])
...
const columns = [
  ...
  { title: '标准工序编号', dataIndex: 'standard_process_number', key: 'standard_process_number', width: 140 },
  { title: '标准工序名称', dataIndex: 'standard_process_name', key: 'standard_process_name', width: 140 },
  { title: '工作中心编号', dataIndex: 'work_center_number', key: 'work_center_number', width: 140 },
  { title: '工作中心名称', dataIndex: 'work_center_name', key: 'work_center_name', width: 140 },
  { title: '车间仓库', dataIndex: 'workshop_warehouse', key: 'workshop_warehouse', width: 100 },
  { title: '是否检验工序', dataIndex: 'inspection_process', key: 'inspection_process', width: 120 },
  { title: '检验方案', dataIndex: 'inspection_schemes', key: 'inspection_schemes', width: 100 },
  { title: '检验规范', dataIndex: 'inspection_specification', key: 'inspection_specification', width: 100 },
  { title: '检验员', dataIndex: 'inspection_process_inspector', key: 'inspection_process_inspector', width: 100 },
  { title: '超额报工', dataIndex: 'over_staffing_reporting', key: 'over_staffing_reporting', width: 100 },
  { title: '超额报工比例', dataIndex: 'excess_reporting_ratio', key: 'excess_reporting_ratio', width: 120 },
  { title: '配料方式', dataIndex: 'ingredient_addition', key: 'ingredient_addition', width: 100 },
  { title: '是否自检', dataIndex: 'self_inspection', key: 'self_inspection', width: 100 },
  { title: '自检检验方案', dataIndex: 'self_inspection_inspection_plan', key: 'self_inspection_inspection_plan', width: 130 },
  { title: '自检规范', dataIndex: 'self_inspection_specification', key: 'self_inspection_specification', width: 100 },
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getProcedures({ page: pagination.current, limit: pagination.pageSize, search: searchText.value })
...
const handleExport = async (format: string = 'xlsx') => {
  try {
    ...
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `procedures.${ext}`; link.click(); URL.revokeObjectURL(link.href)
...
onMounted(() => { fetchData(); fetchWorkCenters() })
</script>
...
<template>
  <div>
    <a-card title="标准工序管理" :bordered="false">
      ...
      <a-table :columns="columns" :data-source="dataSource" :loading="loading" :row-key="(record: Procedure) => record.standard_process_number!" :row-selection="rowSelection" :pagination="pagination" :scroll="{ x: 1200, y: 'calc(100vh - 280px)' }" @change="handleTableChange">
    ...
    <a-modal v-model:open="editModalVisible" title="编辑标准工序" @ok="handleEditOk" okText="确认" cancelText="取消" width="700px">
      <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 14 }">
        ...
        <a-row :gutter="16">
          <a-col :span="12"><a-form-item label="车间仓库"><a-input v-model:value="editForm.workshop_warehouse" /></a-form-item></a-col>
        </a-row>
        <a-row :gutter="16">
    ...
    <a-modal v-model:open="createModalVisible" title="新建标准工序" @ok="handleCreateOk" okText="确认" cancelText="取消" width="700px">
      <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 14 }">
        ...
        <a-row :gutter="16">
          <a-col :span="12"><a-form-item label="车间仓库"><a-input v-model:value="createForm.workshop_warehouse" /></a-form-item></a-col>
        </a-row>
        <a-row :gutter="16">
