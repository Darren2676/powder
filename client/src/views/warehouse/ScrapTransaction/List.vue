...
} = useColumnPreference('scrap_transaction_list', defaultDataColumns, {
  fixedLeft: [],
  fixedRight: []
})
...
const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await getScrapTransactionList({
...
<template>
  <div class="scrap-transaction-page">
      ...
      <div style="margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; flex-wrap: nowrap; overflow-x: auto">
        ...
        <div style="display: flex; gap: 8px; align-items: center; flex-wrap: nowrap">
          ...
          <a-select
            ...
            @change="handleSearch"
          >
            <a-select-option value="报废入库">报废入库</a-select-option>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'source_type'">
            <a-tag :color="record.source_type === '报废入库' ? 'blue' : record.source_type === '报废出库' ? 'orange' : 'default'">
              {{ record.source_type }}
            </a-tag>
          </template>
