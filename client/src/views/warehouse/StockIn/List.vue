...
import { ReloadOutlined, DownloadOutlined, EyeOutlined, DeleteOutlined, CheckCircleOutlined } from '@ant-design/icons-vue'
import { getStockIns, getStockInDetail, deleteStockIn, confirmStockIn, exportStockIns } from '@/api/warehouse/stockIn'
...
const handleConfirm = async (record: any) => {
  try {
    await confirmStockIn(record.stock_in_number)
...
const handleDelete = async (record: any) => {
  await deleteStockIn(record.stock_in_number)
  ...
  <div style="padding: 20px">
    ...
    <a-table :columns="columns" :data-source="dataList" :loading="loading" :pagination="{ current: pagination.current, pageSize: pagination.pageSize, total: pagination.total, showSizeChanger: true, showTotal: (t: number) => `共 ${t} 条` }" @change="handleTableChange" row-key="stock_in_number" :scroll="{ x: 1300 }" size="small">
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'approval_status'">
          <a-tag :color="record.approval_status === '已入库' ? 'green' : 'default'">{{ record.approval_status }}</a-tag>
        </template>
        <template v-else-if="column.key === 'action'">
          <a-space size="small">
            <a-button size="small" @click="openDetail(record)"><template #icon><EyeOutlined /></template>查看</a-button>
            <a-button size="small" type="primary" @click="handleConfirm(record)" :disabled="record.approval_status !== '草稿'"><template #icon><CheckCircleOutlined /></template>确认入库</a-button>
    ...
    <a-modal v-model:open="detailVisible" title="入库单详情" width="1000px" :footer="null">
      <a-descriptions bordered size="small" :column="3" style="margin-bottom:16px">
        ...
        <a-descriptions-item label="状态"><a-tag :color="detailHeader.approval_status === '已入库' ? 'green' : 'default'">{{ detailHeader.approval_status }}</a-tag></a-descriptions-item>
