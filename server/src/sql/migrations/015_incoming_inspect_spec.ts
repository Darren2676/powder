...
async function migrate() {
  try {
    ...
    // 清理旧数据
    await sequelize.query(`DELETE FROM incoming_inspect_spec_item`);
    await sequelize.query(`DELETE FROM incoming_inspect_spec`);
    console.log('已清理旧数据');
    ...
    for (const spec_name of specSeedData) {
      try {
        await sequelize.query(
          `INSERT INTO incoming_inspect_spec (spec_name, defect_categories) VALUES (:spec_name, '')`,
    ...
    // 验证
    const [specCount]: any = await sequelize.query(`SELECT COUNT(*) as cnt FROM incoming_inspect_spec`);
    const [itemCount]: any = await sequelize.query(`SELECT COUNT(*) as cnt FROM incoming_inspect_spec_item`);
    console.log(`最终统计: 主表 ${specCount[0].cnt} 条, 明细 ${itemCount[0].cnt} 条`);

    await sequelize.close();
    console.log('来料检验规范迁移完成');
  } catch (err) {
