  ...
  try {
    ...
    const [orderItems]: any = await sequelize.query(`
      SELECT d.item_number, d.item_name, d.specifications, d.basic_unit, d.product_drawing_number,
             SUM(d.order_quantity - ISNULL(d.shipped_quantity, 0)) AS order_demand
    ...
    for (const [itemNumber, demand] of demandMap) {
      ...
      // 净需求 = MAX(0, 毛需求 - 库存现有量 - 在途量 + 安全库存)
      const netDemand = Math.max(0, demand.gross_demand - inv.on_hand - inTransit + inv.safety_stock);
