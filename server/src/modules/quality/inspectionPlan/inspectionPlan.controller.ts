...
export const updateInspectionPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const [chk]: any = await sequelize.query(`SELECT approval_status FROM inspection_plan WHERE plan_name = :id`, { replacements: { id } });
...
export const deleteInspectionPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await sequelize.query(`DELETE FROM inspection_plan WHERE plan_name = :id`, { replacements: { id } });
...
export const approveInspectionPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    await sequelize.query(`UPDATE inspection_plan SET approval_status = N'已审核' WHERE plan_name = :id`, { replacements: { id } });
...
export const withdrawInspectionPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    await sequelize.query(`UPDATE inspection_plan SET approval_status = N'未审核' WHERE plan_name = :id`, { replacements: { id } });
