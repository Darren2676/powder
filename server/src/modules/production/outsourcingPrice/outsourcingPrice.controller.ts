...
// ==================== 列表 ====================
export const getOutsourcingPriceLists = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [countResult]: any = await sequelize.query(
      `SELECT COUNT(*) as total FROM outsourcing_price_list ${whereClause}`, { replacements }
    );
    ...
    const [items]: any = await sequelize.query(`
      SELECT * FROM (
        ...
        FROM outsourcing_price_list ${whereClause}
      ) AS t
...
export const getOutsourcingPriceListDetail = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [details]: any = await sequelize.query(
      `SELECT * FROM outsourcing_price_list_detail WHERE price_list_number = :id ORDER BY item_number, line_number`,
      { replacements: { id } }
    );
    res.json(success({ header: headers[0], details }, '获取委外价目表详情成功'));
  } catch (err) { next(err); }
};
  ...
  try {
    ...
    try {
      await sequelize.query(`
        INSERT INTO outsourcing_price_list (price_list_number, price_list_name, supplier_number, supplier_name,
      ...
      if (b.details && Array.isArray(b.details)) {
        for (let i = 0; i < b.details.length; i++) {
          const d = b.details[i];
          await sequelize.query(`
            INSERT INTO outsourcing_price_list_detail (price_list_number, line_number, item_number, item_name,
              item_category, specifications, tax_inclusive_price, tax_exclusive_price, tax_rate,
              enable_tiered_pricing, start_quantity, end_quantity, pricing_unit, remark)
      ...
      res.json(success({ price_list_number }, '创建委外价目表成功'));
    } catch (e) {
...
// ==================== 更新 ====================
export const updateOutsourcingPriceList = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const b = req.body;

    const [chk]: any = await sequelize.query(
      `SELECT approval_status FROM outsourcing_price_list WHERE price_list_number = :id`, { replacements: { id } }
    );
    if (!chk.length) { res.status(404).json({ success: false, message: '委外价目表不存在' }); return; }
    ...
    try {
      await sequelize.query(`
        UPDATE outsourcing_price_list SET
      ...
      if (b.details && Array.isArray(b.details)) {
        await sequelize.query(
          `DELETE FROM outsourcing_price_list_detail WHERE price_list_number = :id`,
      ...
      await transaction.commit();
      res.json(success(null, '更新委外价目表成功'));
    } catch (e) {
      await transaction.rollback();
      throw e;
    }
...
// ==================== 删除 ====================
export const deleteOutsourcingPriceList = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const [chk]: any = await sequelize.query(
      `SELECT approval_status FROM outsourcing_price_list WHERE price_list_number = :id`, { replacements: { id } }
    ...
    const transaction = await sequelize.transaction();
    try {
      await sequelize.query(`DELETE FROM outsourcing_price_list_detail WHERE price_list_number = :id`, { replacements: { id }, transaction });
      await sequelize.query(`DELETE FROM outsourcing_price_list WHERE price_list_number = :id`, { replacements: { id }, transaction });
      await transaction.commit();
      res.json(success(null, '删除委外价目表成功'));
    } catch (e) {
      await transaction.rollback();
      throw e;
    }
...
export const exportOutsourcingPriceLists = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const [items]: any = await sequelize.query(`
      ...
      FROM outsourcing_price_list h
      INNER JOIN outsourcing_price_list_detail d ON d.price_list_number = h.price_list_number
      ORDER BY h.price_list_number, d.item_number, d.line_number
    `);
    exportToExcel(items, exportFields, exportHeaders, 'outsourcing_price_lists', res);
  } catch (err) { next(err); }
};
...
export const importOutsourcingPriceList = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      await transaction.commit();
      res.json(success({ price_list_number, imported: rows.length }, `成功导入 ${rows.length} 条明细到委外价目表 ${price_list_number}`));
    } catch (e) {
      await transaction.rollback();
      throw e;
    }
