...
export const updateMouldMaintenance = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const setClauses = fields.map(f => `${f} = :${f}`).join(', ') + ', updated_by = :updated_by';
    const replacements: any = { id, updated_by: username };
    for (const f of fields) {
      replacements[f] = b[f] !== undefined ? b[f] : (f === 'cost' ? 0 : '');
    }
...
export const deleteMouldMaintenance = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    await sequelize.query(`DELETE FROM mould_maintenance WHERE id = :id`, { replacements: { id } });
...
export const exportMouldMaintenances = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [items]: any = await sequelize.query(
      `SELECT mm.*, m.item_name AS mould_name FROM mould_maintenance mm LEFT JOIN mould m ON mm.mould_number = m.item_number ${whereClause} ORDER BY mm.maintenance_date DESC`,
      { replacements }
    );
    exportToExcel(items, fields, headers, 'mould_maintenances', res);
  } catch (err) { next(err); }
};
