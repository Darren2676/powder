...
const tableName = 'product_class';
const pkField = 'product_class_number';
const nameField = 'product_class_name';
const label = '产品分类';
const fields = [pkField, nameField, 'remark'];
const headers = ['产品分类编号', '产品分类名称', '备注'];
...
export const exportData = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    exportToExcel(items, fields, headers, 'product_class', res);
  } catch (err) { next(err); }
};
...
export const approveProductClass = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const withdrawProductClass = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
