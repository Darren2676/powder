...
const tableName = 'material_class';
const pkField = 'material_class_number';
const nameField = 'material_class_name';
const label = '物料分类';
const fields = [pkField, nameField, 'remark'];
const headers = ['物料分类编号', '物料分类名称', '备注'];
...
export const exportData = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    exportToExcel(items, fields, headers, 'material_class', res);
  } catch (err) { next(err); }
};
...
export const approveMaterialClass = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    await sequelize.query(`UPDATE ${tableName} SET approval_status = N'已审核' WHERE ${pkField} = :id`, { replacements: { id } });
    res.json(success(null, '审核成功'));
  } catch (err) { next(err); }
};

export const withdrawMaterialClass = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    await sequelize.query(`UPDATE ${tableName} SET approval_status = N'未审核' WHERE ${pkField} = :id`, { replacements: { id } });
