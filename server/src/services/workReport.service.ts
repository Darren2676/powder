...
const autoCreateInspections = async (taskNo: string, wrNumber: string, totalQty: number, username: string, transaction: any) => {
    const [taskRows]: any = await sequelize.query(
      `SELECT process_task_number, production_order_number, step_number, standard_process_name, item_number, item_name, specifications, enable_self_inspect, enable_special_inspect, self_inspect_plan_name, special_inspect_plan_name, self_inspect_spec_name, special_inspect_spec_name FROM process_task WHERE process_task_number = :taskNo`,
    ...
    if (t.enable_self_inspect === '是') {
      const num = await createInspectionFromWorkReport({
        ...
        inspect_type: '自检',
        inspection_plan_name: t.self_inspect_plan_name || '',
        inspection_spec_name: t.self_inspect_spec_name || '',
    ...
    if (t.enable_special_inspect === '是') {
      const num = await createInspectionFromWorkReport({
        ...
        specifications: t.specifications || '',
        inspect_type: '专检',
        inspection_plan_name: t.special_inspect_plan_name || '',
        inspection_spec_name: t.special_inspect_spec_name || '',
        total_quantity: totalQty,
        creation_man: username
      }, transaction);
      inspectionNumbers.push(num);
    ...
    if (inspectionNumbers.length > 0) {
      console.log(`[报工${wrNumber}] 自动创建检验记录: ${inspectionNumbers.join(', ')}`);
    }
};
...
export const syncTaskCompletion = async (taskNo: string, qtyDelta: number, transaction?: any): Promise<void> => {
  ...
  try {
    ...
    if (orderNo) {
      ...
      if (parseInt(pendingTasks[0]?.cnt) === 0) {
        await sequelize.query(
...
export const completeOrderReport = async (params: {
  ...
  return await withTransaction(async (transaction) => {
    ...
    if (qualifiedQty > 0) {
      ...
      await syncTaskCompletion(lastTask.process_task_number, qualifiedQty, transaction);

      // 线边仓流转记录
      await logWorkReportLinesideMovement(lastTask.process_task_number, qualifiedQty, wrNumber, user?.username || '', transaction);
    }
