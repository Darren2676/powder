...
export const createWorkCenter = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { work_cente_number, work_cente_name, remark } = req.body;
    if (!work_cente_number) { res.status(400).json({ success: false, message: '工作中心编号不能为空' }); return; }
    await sequelize.query(`INSERT INTO work_center (work_cente_number, work_cente_name, [condition], remark) VALUES (:work_cente_number, :work_cente_name, :condition, :remark)`, {
      replacements: { work_cente_number, work_cente_name: work_cente_name || '', condition: req.body.condition || '', remark: remark || '' }
    });
...
export const updateWorkCenter = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { work_cente_name, remark } = req.body;
    await sequelize.query(`UPDATE work_center SET work_cente_name = :work_cente_name, [condition] = :condition, remark = :remark WHERE work_cente_number = :id`, { replacements: { id, work_cente_name, condition: req.body.condition || '', remark } });
