...
export const createConversion = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { from_unit_code, to_unit_code, conversion_rate, remark } = req.body;
    ...
    const [existing]: any = await sequelize.query(
      ...
      { replacements: { from: from_unit_code, to: to_unit_code } }
    );
    ...
    await sequelize.query(
      ...
      { replacements: { from_unit_code, to_unit_code, conversion_rate, remark: remark || '' } }
    );
...
export const updateConversion = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { conversion_rate, remark } = req.body;
    ...
    await sequelize.query(
      ...
      { replacements: { id, conversion_rate, remark: remark || '' } }
    );
