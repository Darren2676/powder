  ...
  <div class="page-container">
    ...
    <van-popup v-model:show="scannerVisible" position="bottom" round :style="{ height: '75%' }">
      <div class="scanner-container">
        ...
        <div id="qr-reader-issue" class="scanner-view"></div>
    ...
    <div v-if="orderInfo" class="order-info card">
      <van-cell-group inset>
        ...
        <van-cell title="已发数量" :value="formatNum(orderInfo.issued_quantity)" />
        <van-cell title="发料状态">
          <template #value>
            <van-tag :type="getStatusType(orderInfo.issue_status)">{{ orderInfo.issue_status }}</van-tag>
          </template>
        </van-cell>
      </van-cell-group>
      ...
      <div class="progress-bar">
        <div class="progress-label">发料进度</div>
        <van-progress
          :percentage="getProgressPercent()"
          stroke-width="8"
          :show-pivot="true"
          :pivot-text="`${orderInfo.issued_quantity}/${orderInfo.planned_quantity}`"
        />
      </div>
    </div>
    ...
    <div v-if="orderInfo" class="bottom-bar">
      ...
      <van-button type="primary" block :disabled="!canSubmit" @click="submitIssue">
        确认发料 ({{ getTotalIssued() }}/{{ getTotalPlanned() }})
      </van-button>
    </div>
...
const loadOrder = async () => {
  ...
  try {
    const res = await axios.get(`/api/v1/mobile/outsourcing/issue/${orderNumber.value}`)
    if (res.data.success) {
      ...
      // 缓存订单数据
      await setCachedData(`issue_order_${orderNumber.value}`, res.data.data, 24 * 60 * 60 * 1000)
    ...
    // 网络错误时尝试从缓存读取
    if (!isOnline.value) {
      const cached = await getCachedData(`issue_order_${orderNumber.value}`)
      if (cached) {
...
const openScanner = () => {
  ...
  scannedItems.value = []
  setTimeout(() => {
    qrReader = new Html5Qrcode('qr-reader-issue')
    qrReader.start(
    ...
    if (data.type === 'outsourcing_order' || data.t === 'os') {
      ...
      if (item) {
        if (batchMode.value) {
          ...
          // 单扫模式：直接填充数量并关闭
          item.issued_qty = item.remaining_quantity
          closeScanner()
          speakScanSuccess(item.item_name)
...
const applyBatchScan = () => {
  scannedItems.value.forEach((scanned) => {
    ...
    if (item) {
      item.issued_qty = scanned.scan_qty
    }
  })
...
const canSubmit = computed(() => {
  ...
  return getTotalIssued() > 0
})

const getTotalIssued = () => {
  if (!orderInfo.value) return 0
  return orderInfo.value.items.reduce((sum: number, item: any) => sum + (item.issued_qty || 0), 0)
}
...
const getProgressPercent = () => {
  if (!orderInfo.value) return 0
  return Math.round((orderInfo.value.issued_quantity / orderInfo.value.planned_quantity) * 100)
}
...
const submitIssue = async () => {
  ...
  const items = orderInfo.value.items
    .filter((item: any) => item.issued_qty > 0)
    .map((item: any) => ({
  ...
  try {
    ...
    if (!isOnline.value) {
      await enqueueRequest({
        method: 'POST',
        url: '/mobile/outsourcing/issue',
        body: JSON.stringify(payload),
        description: `委外发料 ${orderInfo.value.order_number}`,
      })
    ...
    const res = await axios.post('/api/v1/mobile/outsourcing/issue', payload)

    if (res.data.success) {
      showSuccessToast('发料成功')
      speakSuccess('发料成功')
      setTimeout(() => {
        router.back()
      }, 1500)
    } else {
      showToast(res.data.message || '发料失败')
      speakScanFailed('发料失败')
    }
  } catch (error: any) {
    if (error !== 'cancel') {
      // 网络错误时入队
      if (!isOnline.value || error.code === 'ERR_NETWORK') {
        ...
        showToast(error.response?.data?.message || '发料失败')
      }
    }
  } finally {
    closeToast()
