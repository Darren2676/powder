      ...
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
        ...
        >
          <div
            ...
            @click="goProcessPrep(order)"
          >
            <div class="order-card__header">
              <span class="order-card__number">{{ order.production_order_number }}</span>
              <span class="status-tag status-tag--success">已派发</span>
            </div>
            <div class="order-card__body">
              ...
              <div class="order-card__info">
                ...
                <div class="info-item">
                  ...
                  <span :class="['info-value', order.bom_matched ? 'text-success' : 'text-danger']">
                    {{ order.bom_matched ? order.bom_version || '已匹配' : '无BOM' }}
                  </span>
                </div>
                <div class="info-item">
                  <span class="info-label">已有备料单</span>
                  <span class="info-value">{{ order.existing_preparation_count || 0 }}张</span>
                </div>
              </div>
            </div>
            <div class="order-card__footer">
              <van-button size="small" type="primary" block round @click.stop="goProcessPrep(order)">
                开始按工序备料
              </van-button>
            </div>
          </div>
...
import { getOrdersForProcessPrep } from '@/api/materialPreparation'
...
const fetchOrders = async (isRefresh = false) => {
  ...
  loading.value = true
  try {
    const res: any = await getOrdersForProcessPrep({
      page: page.value,
      limit,
...
const onScanSuccess = (decodedText: string) => {
  ...
  // 解析二维码内容，提取生产单号
  // 二维码可能直接是生产单号，也可能是包含生产单号的JSON或URL
  let orderNo = decodedText.trim()

  // 尝试解析JSON格式 { production_order_number: "xxx" }
  try {
  ...
  if (orderNo) {
    ...
    // 直接跳转到该生产单的按工序备料操作页
    router.push(`/process-prep/${encodeURIComponent(orderNo)}`)
  } else {
...
const goProcessPrep = (order: any) => {
  router.push(`/process-prep/${encodeURIComponent(order.production_order_number)}`)
}
...
.text-danger { color: var(--danger, #ee0a24) !important; }
